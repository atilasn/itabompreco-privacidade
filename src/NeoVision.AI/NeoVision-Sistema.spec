...
# PyInstaller: gera `dist/NeoVision-Sistema/NeoVision-Sistema.exe` (pasta com DLLs).
# API + painel web + janela pywebview (WebView2 no Windows).
# Executar:  cd este diretório  ;  pyinstaller -y NeoVision-Sistema.spec
# Ou:  powershell -ExecutionPolicy Bypass -File repo\build\publish-sistema-desktop-python.ps1
...
# ONVIF (onvif-zeep): pasta `wsdl` ao lado do pacote na instalação pip.
